...
class WP_GIF_Player{
    ...
    function shortcodes($atts){
        ...
        if($gif_id != null && !empty($gif_id) ){
            if(get_post_mime_type($gif_id) === "image/gif"){
                ...
                $gif_attach = '';
                $gif_alt = get_post_meta($gif_id, '_wp_attachment_image_alt', true);
                ...
                $output = '<div class="gif_wrap ' . $width . '">
                        <a href="javascript:void(0);" class="gif_link_wrap ' . $width . '" title="Click to play" rel="nofollow"></a>
                        <span class="play_gif ' . $width . '">GIF</span>
                        <img src="' . $still_attach . '" class="_showing frame no-lazy" alt="'.$gif_alt.'">
                   </div>
                   <img src="' . $still_attach . '" class="_hidden no-lazy" alt="'.$gif_alt.'" style="display:none;">';
            }
        }else{
            $output = '<p>' . __('Sorry, but we couldn\'t find a GIF with this ID!', 'WPGP') . '</p>';
        }
        return $output;
    }
    ...
    */
    ...
    {
        __('Choose your settings below:', 'WPGP');
    }
